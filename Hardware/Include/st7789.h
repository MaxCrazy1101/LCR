...
#define USE_HORIZONTAL    0 // 1或2 为竖屏 3或4为横屏
...
// GPIO_ResetBits(GPIOA, GPIO_Pin_3) // CS
#define LCD_CS_Clr()
...
void st7789_transmit_data8(uint8_t dat);                                     // 写入一个字节
void st7789_transmit_data16(uint16_t dat);                                   // 写入2个字节
void st7789_transmit_cmd(uint8_t dat);                                       // 写入寄存器
void st7789_set_address(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2); // 设置目标地址
